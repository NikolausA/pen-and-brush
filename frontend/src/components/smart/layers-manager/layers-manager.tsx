...
export const LayersManager = ({
  ...
  const { addLayerCreatedHistory, addLayerDeletedHistory } =
    useHistoryManager(projectId);
  ...
  const handleCreateLayer = useCallback(async () => {
    try {
      // Находим максимальный order среди существующих слоев
      const maxOrder = layersData.reduce(
      ...
      const newLayerData = {
        ...
        order: maxOrder + 1, // Новый слой всегда поверх
        isVisible: true,
        opacity: 100, // Новый слой полностью непрозрачный
        data: [],
      };
  ...
  const handleDeleteLayer = useCallback(
    async (layerId: string) => {
      ...
      try {
        ...
        // Если удаляем активный слой, выбираем слой с максимальным order
        if (activeLayerId === layerId) {
          const remainingLayers = updatedLayers.sort(
  ...
  const handleRenameLayer = useCallback(
    async (layerId: string, newName: string) => {
      const layer = layersData.find((l) => l.id === layerId);
  ...
  // Изменение прозрачности с дебаунсом
  const handleOpacityChange = useCallback(
    async (opacity: number) => {
      ...
      try {
        ...
        console.log("Layer opacity changed:", activeLayer.id, clampedOpacity);

        // Принудительное обновление для применения прозрачности к объектам
        refetch();
      } catch (error) {
        ...
        <Pane fontSize="12px" color="#6B7280">
          ...
          {activeLayer && (
            <Pane marginTop={4}>
              ...
              <Pane marginTop={2} color="#8B949E">
                Порядок: {activeLayer.order || 0} (выше = поверх)
              </Pane>
            </Pane>
          )}
        </Pane>
