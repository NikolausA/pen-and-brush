...
router.patch("/projects/:projectId", async (req, res) => {
  try {
    ...
    if (req.body.layers && Array.isArray(req.body.layers)) {
      const incomingLayerIds = req.body.layers
        .map((layer) => layer.id)
        .filter(Boolean);
...
router.get("/layers", async (req, res) => {
  try {
    ...
    if (!projectId) {
      return res
        .status(400)
        .json({ error: "Bad request", message: "projectId is required" });
    }
...
router.post("/layers", async (req, res) => {
  try {
    ...
    if (!projectId || !isUUID(projectId)) {
      return res
        .status(400)
        .json({ error: "Bad request", message: "Valid projectId is required" });
    }

    if (!name || typeof order !== "number") {
      return res
        .status(400)
        .json({ error: "Bad request", message: "Name and order are required" });
    }
...
router.patch("/layers/:layerId", async (req, res) => {
  try {
    ...
    const { projectId, data, name, order, isVisible, opacity } = req.body;

    if (!isUUID(layerId)) {
      return res
        .status(400)
        .json({ error: "Bad request", message: "Invalid layerId format" });
    }
    ...
    console.log("🔧 [BACKEND] Updating layer:", layerId);
    console.log("🔧 [BACKEND] Request body keys:", Object.keys(req.body));

    // КРИТИЧНО: Для JSONB полей Sequelize нужно явно указать changed
    if (data !== undefined) {
      console.log(
        "🔧 [BACKEND] Updating data field, length:",
        Array.isArray(data) ? data.length : "N/A"
      );
      layer.data = data;
      layer.changed("data", true);
    }

    // Обновляем остальные поля
    if (name !== undefined) layer.name = name;
    if (order !== undefined) layer.order = order;
    if (isVisible !== undefined) layer.isVisible = isVisible;
    ...
    console.log("✅ [BACKEND] Layer updated successfully:", layerId);
    console.log(
      "✅ [BACKEND] Final data field length:",
      Array.isArray(layer.data) ? layer.data.length : "N/A"
    );

    res.json(layer);
  } catch (error) {
    console.error("❌ [BACKEND] Error updating layer:", error);
    res
      .status(500)
      .json({ error: "Internal server error", message: error.message });
  }
...
router.get("/history", async (req, res) => {
  try {
    ...
    console.log(
      `Found ${history.length} history records for project ${projectId}`
    );
    res.json(history);
  } catch (error) {
...
router.post("/history", async (req, res) => {
  try {
    ...
    if (!action || typeof action !== "string") {
      return res
        .status(400)
        .json({ error: "Bad request", message: "Action is required" });
    }
...
router.delete("/history/:historyId", async (req, res) => {
  try {
    ...
    if (!isUUID(historyId)) {
      return res
        .status(400)
        .json({ error: "Bad request", message: "Invalid historyId format" });
    }
